...
void calc_mb_onearg(real centerX, real centerY, const real magn, const int max_iter, const float colorDensity, const float colorOffset,
	int* current_passes, const int max_passes, std::vector<float>* out_vec_img_f, int* out_width, int* out_height, bool* stop, std::mutex*)
{
	...
	for (int pass = 0; pass < max_passes; pass++)
	{
		...
		for (int y = 0; y < imgHeight; y++) 
		{
			for (int x = 0; x < imgWidth; x++) 
			{
				...
				const real hash_random = uintToUnitReal(hash(pass * imgWidth * imgHeight + pixel_idx)); // Use pixel idx and pass to randomise Halton sequence
				real x_offset = min(max_passes - 1, 1) * triDist(wrap1r(halton<2>(pass), hash_random));
				real y_offset = min(max_passes - 1, 1) * triDist(wrap1r(halton<3>(pass), hash_random));
				real x_shifted = (real)x + x_offset + 0.5;
				real y_shifted = (real)y + y_offset + 0.5;
				//if (x == 0 && y == 0)
				//{
				...
				color pixel_color = main_iter_loop2(z, colorDensity, colorOffset, max_iter);
